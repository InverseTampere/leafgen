...
% Use all three marginal LADD (or point cloud) in positioning
elseif isa(fDist_h,"function_handle") && isa(fDist_d,"function_handle") ...
       && isa(fDist_c,"function_handle")

    accepted = 0;
    while accepted == 0
        % Sample relative height from LADD
        relHeight = rand(1);
        if fDist_h(relHeight) > rand(1)*maxfDist_h
            hLeaf = relHeight*(zMax-zMin) + zMin;
        else 
        ...
        if PCSampling.ratio(iPCS) <= PCSampling.weights(iPCS) ...
            ...
            zSliceCol = zSlice(:);
            zSliceIndsCol = zSliceInds(:);
            % If no voxels on height level, do LADD sampling
            if sum(zSliceCol) == 0
                sampleByLADD = true;
            else
                cumulativeSum = cumsum(zSliceCol);
        ...
        if sampleByLADD == true
            hProposal = relHeight;
            % Proposal values to relative distance and compass direction
            dProposal = rand(1);
            cProposal = 2*pi*rand(1);
            % Marginal LADD values on proposal points
            funValues = [fDist_d(dProposal),fDist_c(cProposal)];
            vertValues = rand(1,2).*[maxfDist_d,maxfDist_c];
