...
function Leaves = add_leaves_cylinder(Leaves,len,rad,inc,az,ar, ...
...
cylAxisStartPoint = zeros(1,3);
cylAxis = [sin(inc)*cos(az+pi/2), ...
           sin(inc)*sin(az+pi/2), ...
           cos(inc)];
cylAxis = cylAxis/norm(cylAxis);

% Cylinder attributes
sp = cylAxisStartPoint;
ax = cylAxis;
par = 0;
BI  = 0;

QSM = QSMBCylindrical(sp,ax,len,rad,par,BI);
