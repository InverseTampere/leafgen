...
for iCyl = 1:length(cylinderLeafArea)
    ...
    p_inc = TargetDistributions.fun_inc_params( ...
                CylinderParameters.relative_height, ...
                CylinderParameters.relative_distance_along_subbranch, ...
                CylinderParameters.compass_direction);
    % Leaf azimuth angle distribution parameter values for the cylinder
    p_az  = TargetDistributions.fun_az_params( ...
                CylinderParameters.relative_height, ...
                CylinderParameters.relative_distance_along_subbranch, ...
                CylinderParameters.compass_direction);
    % LOD nodes
    [~,iLodInc1] = find_closest_node(Nodes.LodInc1,p_inc(1));
    [~,iLodInc2] = find_closest_node(Nodes.LodInc2,p_inc(2));
    [~,iLodAz1]  = find_closest_node(Nodes.LodAz1,p_az(1));
    [~,iLodAz2]  = find_closest_node(Nodes.LodAz2,p_az(2));

    % LEAF SIZE DISTRIBUTION
    % Leaf size distribution parameter values for the cylinder
    p_size = TargetDistributions.fun_size_params( ...
                CylinderParameters.relative_height, ...
                CylinderParameters.relative_distance_along_subbranch, ...
                CylinderParameters.compass_direction);
    [~,iLsd1] = find_closest_node(Nodes.Lsd1,p_size(1));
    [~,iLsd2] = find_closest_node(Nodes.Lsd2,p_size(2));


    % PICK CORRESPONDING LEAF OBJECT FROM THE LIBRARY
    iNodeObject = randi(LeafCylinderLibrary.nLeafObjectsPerNode);
    LibraryObj = LeafCylinderLibrary.LeavesObjects{iLen,iRad,iInc,iAz, ...
                                                   iAr,iLodInc1, ...
                                                   iLodInc2,iLodAz1, ...
                                                   iLodAz2,iLsd1, ...
                                                   iLsd2,iNodeObject};
    ...
    if cylinderLeafArea(iCyl) > LibraryObj.leaf_area
        ...
        while remainingArea > cylinderLeafArea(iCyl)
            remainingArea = remainingArea - leafBaseArea ...
                            *leafScales(end-k,1)*leafScales(end-k,2);
            k = k + 1;
        end
    ...
    if iCyl == 1 && endIndex > 0
        ...
        Leaves.leaf_area = sum(leafBaseArea*leafScales(1:endIndex,1) ...
                               .*leafScales(2:endIndex,1));
    ...
    elseif endIndex > 0
        ...
        Leaves.leaf_area = Leaves.leaf_area + sum(leafBaseArea ...
                                              *leafScales(1:endIndex,1) ...
                                              .*leafScales(1:endIndex,2));
