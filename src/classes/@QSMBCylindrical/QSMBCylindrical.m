...
% Fixed the "translate" method to compute translations for the cylinder 
% mid points, cylinder end points, the branch heights, and the tree 
% bounding box. Previously the translation was applied only to cylinder
% start points.
...
classdef QSMBCylindrical < QSMB
    ...
    methods
        ...
        % Move cylinder start, mid, and end points by given delta-vector,
        % update branch heights based on z-component of delta, and compute
        % new tree bounding box
            ...
            if ob.compressed == false
                % Move cylinder end points
                ob.cylinder_end_point(1:ob.block_count,:) = bsxfun( ...
                    @plus, ...
                    ob.cylinder_end_point(1:ob.block_count,:), ...
                    Delta ...
                );

                % Move cylinder mid points
                ob.cylinder_mid_point(1:ob.block_count,:) = bsxfun( ...
                    @plus, ...
                    ob.cylinder_mid_point(1:ob.block_count,:), ...
                    Delta ...
                );

                % Update branch heights
                ob.branch_height = bsxfun(@plus, ...
                    ob.branch_height, ...
                    Delta(3) ...
                );

                % Compute new tree bounding box
                Ep_pr = bsxfun( ...
                    @plus, ...
